...
def scoreboard(league, game_date):
	...
	if league == 'nhl':
		json_data = clean_nhl(data)
	else:
		json_data = clean_default(data)
...
def clean_default(data):
	...
	data = data.text[start:end].replace(";", "").replace("window.espn.scoreboardData", "").replace(
	...
	try:
		...
		for game in json_data:
			...
			if len(game['broadcasts']) > 0:
				game['broadcast'] = game['broadcasts'][0]
			del game['id']
			...
			if 'note' in game:
				del game['note']
			if 'completed' in game:
				del game['completed']
			if 'tickets' in game:
				del game['tickets']
			if 'links' in game:
				del game['links']
			if 'hideScoreDate' in game:
			...
			if 'leaders' in game:
				del game['leaders']
			if 'gameTimeFormat' in game:
				del game['gameTimeFormat']
			if 'performerTitle' in game:
